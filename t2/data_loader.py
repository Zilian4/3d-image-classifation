...
def get_data_list(root='D:\DATASET\IPMN_t2_cyst', t = 1, center = None):
    image_list = []
    label_list = []
    center_names = [['nyu'], ['CAD', 'MCF'], ['northwestern', 'NU'], ['AHN', 'ahn'], ['mca'], ['IU'], ['EMC']]
    
    df = pd.read_csv(os.path.join(root, 'ipmn_labels_train'+'.csv'))
    df_cleaned = df.dropna(subset=[df.columns[1]]) # remove NaN
    names = df_cleaned.iloc[:, 0].values
    labels = df_cleaned.iloc[:, 1].to_numpy(dtype=int)
    if center == None:
    ...
    for i in range(len(names)):
        ...
        for c in center_name:
            if c in name:
                image_list.append(os.path.join(root, 'ROI_train', name+'.nii.gz'))
